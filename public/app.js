...
const accessKeyPanel = document.querySelector("#access-key-panel");
const accessKeyAccount = document.querySelector("#access-key-account");
const accessKeyAccountName = document.querySelector("#access-key-account-name");
const accessKeyAccountEmail = document.querySelector("#access-key-account-email");
const getAccessKeyButton = document.querySelector("#get-access-key");
const accessKeyLogoutButton = document.querySelector("#access-key-logout");
...
let lojaUrlAtual = "https://loja-dayzombi.onrender.com";
...
getAccessKeyButton.addEventListener("click", abrirLojaParaObterChave);
accessKeyLogoutButton.addEventListener("click", sairDaConta);
...
async function verificarContaEContinuar() {
  ...
  try {
    ...
    if (!conta.temAcessoTeste) {
      mostrarPainelChaveNecessaria(conta.lojaUrl);
      return;
    }
...
async function prepararAberturaDoApp() {
  ...
  try {
    resultado = await buscarJsonAutenticado("/api/criar-codigo-login-app", {
      method: "POST",
      body: JSON.stringify({
    ...
    if (error?.dados?.precisaChave) {
      mostrarPainelChaveNecessaria(error.dados.lojaUrl);
      return;
    }
    throw error;
  }
...
function mostrarPainelChaveNecessaria(lojaUrl) {
  ultimoLinkApp = null;
  openAppButton.hidden = true;
  ...
  lojaUrlAtual =
    String(lojaUrl || configuracao?.lojaUrl || "").trim() ||
    "https://loja-dayzombi.onrender.com";

  loginPanel.hidden = true;
  accessKeyPanel.hidden = false;

  if (usuarioAtual) {
    accessKeyAccount.hidden = false;
    accessKeyAccountName.textContent = usuarioAtual.displayName || "Jogador DayZombi";
    accessKeyAccountEmail.textContent = usuarioAtual.email || "";
  } else {
    accessKeyAccount.hidden = true;
    accessKeyAccountName.textContent = "";
    accessKeyAccountEmail.textContent = "";
  }
}

function mostrarPainelLogin() {
  accessKeyPanel.hidden = true;
  loginPanel.hidden = false;
}

function abrirLojaParaObterChave() {
  window.location.href = lojaUrlAtual || "https://loja-dayzombi.onrender.com";
}
...
async function buscarJson(url, opcoes = {}) {
  ...
  if (!response.ok) {
    const error = new Error(data.erro || "O servidor recusou a solicitação.");
    error.dados = data;
    throw error;
  }
...
async function buscarJsonAutenticado(url, opcoes = {}) {
  ...
  // Reutiliza o token enquanto ele for válido. O SDK renova automaticamente
  // quando necessário; forçar refresh em cada chamada gerava tráfego extra.
