...
async function verificarContaEContinuar() {
  ...
  try {
    ...
    if (!conta.temAcessoTeste) {
      redirecionarParaLojaSemChave(conta.lojaUrl);
      return;
    }
...
async function prepararAberturaDoApp() {
  ...
  try {
    ...
    if (error?.dados?.precisaChave) {
      redirecionarParaLojaSemChave(error.dados.lojaUrl);
      return;
    }
    throw error;
  }
...
function redirecionarParaLojaSemChave(lojaUrl) {
  ultimoLinkApp = null;
  openAppButton.hidden = true;
  ...
  mostrarStatus(
    "É necessário obter uma chave de acesso nesta conta. Redirecionando para a loja…",
    true
  );
