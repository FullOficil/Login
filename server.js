...
app.get("/api/minha-conta", async (req, res) => {
  ...
  try {
    ...
    const [snapshot, acessoTeste] = await Promise.all([
      contaRef.once("value"),
      obterStatusAcessoTeste(usuario.uid)
    ]);
...
app.post("/api/criar-codigo-login-app", async (req, res) => {
  ...
  try {
    ...
    const acessoTeste = await obterStatusAcessoTeste(usuario.uid);

    if (!acessoTeste.temAcessoTeste) {
...
app.post("/api/trocar-codigo-login-app", async (req, res) => {
  ...
  try {
    ...
    if (temAcessoTeste) {
      try {
        apoiadores = await obterNicksDeQuemApoiouFinanceiramente();
      } catch (error) {
        console.warn(
          "Não foi possível carregar a lista de apoiadores:",
          resumirErro(error)
        );

        const nickAtual = limparNickPublico(
          dadosConta.nick || registro.nick || ""
        );

        if (nickAtual) {
          apoiadores = [nickAtual];
        }
      }
    }
...
async function obterNicksDeQuemApoiouFinanceiramente() {
  if (!firebaseDb) return [];

  // Fonte principal: a loja só mantém em ApoiosPublicos pagamentos que foram
  // confirmados e contabilizados. Isso inclui tanto doações quanto compras.
  // A fonte de contas serve como compatibilidade para registros financeiros
  // antigos que já estejam no Resumo da conta, mas não em ApoiosPublicos.
  const [apoiosSnapshot, loginsSnapshot] = await Promise.all([
    firebaseDb
      .ref(`${FIREBASE_LOJA_PATH}/ApoiosPublicos`)
      .once("value"),
    firebaseDb
  ...
  const apoiosPublicos = apoiosSnapshot.val() || {};
  const usuariosLogin = loginsSnapshot.val() || {};
  const nicksUnicos = new Map();

  // Inclui qualquer pessoa que tenha feito uma doação ou comprado o acesso.
  for (const registro of Object.values(apoiosPublicos)) {
    const tipo = registro?.tipoApoio === "compra" ? "compra" : "doacao";
    const valorCentavos = Number(registro?.valorCentavos) || Math.round(
      Number(registro?.valor || 0) * 100
    );

    if ((tipo !== "compra" && tipo !== "doacao") || valorCentavos <= 0) {
      continue;
    }

    const nick = limparNickPublico(
      registro?.nome ||
      registro?.nick ||
      registro?.nomeApoiador ||
      ""
    );

    adicionarNickUnico(nicksUnicos, nick);
  }

  // Compatibilidade: contas com contribuição financeira aprovada registrada
  // no resumo também entram, mesmo que o apoio público antigo esteja ausente.
  for (const conta of Object.values(usuariosLogin)) {
    const resumo = conta?.Eventos?.Resumo || {};
    const totalContribuido = Number(resumo.totalContribuido) || 0;
    const valorDoado = Number(resumo.valorDoado) || 0;
    const valorApoios = Number(resumo.valorApoios) || 0;
    const quantidadeDoacoes = Number(resumo.quantidadeDoacoes) || 0;
    const quantidadeCompras = Number(resumo.quantidadeCompras) || 0;
    const comprouAcessoTeste = resumo.comprouAcessoTeste === true;

    const apoiouFinanceiramente =
      totalContribuido > 0 ||
      valorDoado > 0 ||
      valorApoios > 0 ||
      quantidadeDoacoes > 0 ||
    ...
    if (!apoiouFinanceiramente) continue;

    const nick = limparNickPublico(conta?.Dados?.nick || "");
    adicionarNickUnico(nicksUnicos, nick);
  }

  const nicks = [...nicksUnicos.values()];

  // Mistura a lista no servidor. A Unity também escolhe aleatoriamente durante
  // o carregamento, então a ordem nunca fica repetitiva.
  for (let i = nicks.length - 1; i > 0; i--) {
    const j = crypto.randomInt(0, i + 1);
    [nicks[i], nicks[j]] = [nicks[j], nicks[i]];
  }

  return nicks.slice(0, 300);
}

function adicionarNickUnico(mapa, nick) {
  if (!nick) return;

  const chaveUnica = nick.toLocaleLowerCase("pt-BR");
  if (!mapa.has(chaveUnica)) {
    mapa.set(chaveUnica, nick);
  }
}
