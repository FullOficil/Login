...
app.get("/api/minha-conta", async (req, res) => {
  ...
  try {
    ...
    const acessoTeste = await obterStatusAcessoTesteAutenticado(usuario);

    // Conta sem acesso não precisa baixar nem mesmo o nó da conta.
    // A decisão vem da Custom Claim do Firebase Auth.
...
app.post("/api/trocar-codigo-login-app", async (req, res) => {
  ...
  try {
    ...
    // Códigos novos já carregam o resultado de acesso validado no momento em
    // que foram criados. Assim a Unity não precisa fazer outra leitura da conta
    // nem de AcessoTeste para concluir o login.
    let nickConta = limparNickPublico(registro.nick || "");
    let temAcessoTeste = registro.temAcessoTeste === true;
    let possuiChave = registro.possuiChave === true;
    let acessoAtivo = registro.acessoAtivo === true;
    let compradoEm = Number(registro.compradoEm) || null;
