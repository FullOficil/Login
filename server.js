...
app.post("/api/criar-codigo-login-app", async (req, res) => {
  ...
  try {
    ...
    if (sessaoEditor) {
      const sessaoHash = criarHash(sessaoEditor);
      gravacoes.push(
        firebaseDb
          .ref(`${FIREBASE_LOGIN_APP_PATH}/EDITOR_SESSOES/${sessaoHash}`)
          .set({
            codigo,
            criadoEm: agora,
...
app.get("/api/consultar-login-editor", async (req, res) => {
  ...
  try {
    const transacao = await sessaoRef.transaction((registroAtual) => {
      if (!registroAtual) {
        resultado = "aguardando";
        return;
      }

      if (registroAtual.entregue === true) {
        resultado = "entregue";
        return;
      }
    ...
    if (resultado === "entregue") {
      return res.status(410).json({
        pronto: false,
        erro: "Este retorno do Editor já foi entregue."
      });
    }
    ...
    if (!transacao.committed || !registro?.codigo) {
      return res.json({ pronto: false });
    }
...
app.post("/api/trocar-codigo-login-app", async (req, res) => {
  ...
  try {
    ...
    const possuiChave = typeof acesso.chave === "string" && acesso.chave.trim().length > 10;
