...
app.post("/api/trocar-codigo-login-app", async (req, res) => {
  ...
  try {
    ...
    if (temAcessoTeste) {
      try {
        apoiadores = await obterNicksApoiadoresAtivos();
      } catch (error) {
        console.warn(
...
async function obterNicksApoiadoresAtivos() {
  if (!firebaseDb) return [];

  const [lojaSnapshot, loginsSnapshot] = await Promise.all([
    firebaseDb
      .ref(`${FIREBASE_LOJA_PATH}/Usuarios`)
      .once("value"),
    firebaseDb
  ...
  const usuariosLoja = lojaSnapshot.val() || {};
  const usuariosLogin = loginsSnapshot.val() || {};
  const nicksUnicos = new Map();

  for (const [uid, registroUsuario] of Object.entries(usuariosLoja)) {
    const acesso = registroUsuario?.AcessoTeste || {};
    const possuiChave =
      typeof acesso.chave === "string" &&
      acesso.chave.trim().length > 10;
    ...
    if (!nicksUnicos.has(chaveUnica)) {
      nicksUnicos.set(chaveUnica, nick);
    }
  }
